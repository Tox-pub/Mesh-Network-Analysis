...
SUITES = [
    ...
    ('test_benchmark_layout.py',
     ...
     []),
    ('test_disk_accounting.py',
     'the Database screen accounts for other projects and folder totals',
     []),
    ('test_database_table.py',
     'the data table headings sit over their values, and report a build date',
     []),
    ('test_settings_buttons.py',
     'the action buttons stay on screen on every tab, even in a short window',
     []),
    ('test_help_menu.py',
     'the Help menu opens the documents in a browser and falls back to a reader',
     []),
    ('test_manual_html.py',
     'the manual renders as a page whose table of contents actually jumps',
     []),
    ('test_strata_and_vocabulary.py',
     'any annotation scheme is valid, and the MeSH trees are a choice',
     []),
    ('test_stopword_rebuild.py',
     'the rebuild writes a terms file the tree checkboxes can actually use',
     []),
    ('test_gui_surface.py',
     'the window shows what it should - bold step, short help pane, one results button',
     []),
    ('test_gui_behaviour.py',
     'the sync dialog and the post-abort damage check actually run',
     []),
    ('test_prompt_and_prefix.py',
     'a fresh prefix is not warned, and a prompt with no console does not kill the run',
     []),
    ('test_citation_fallback.py',
     'secondary analysis generates the citation data it needs when none was retrieved',
     []),
    ('test_build_location.py',
     'every build script writes to one place, or stops rather than guessing',
     []),
    ('test_installer_commands.py',
     'every installer command line parses, and no prompt can hang an uninstall',
     []),
    ('test_uninstall_advice.py',
     'the uninstaller tells each install shape how to actually remove itself',
     []),
