...
def run_community_detection(network_file_path: str, random_seed: int,
    ...
    They are computed unconditionally, so every weighting the benchmark and the
    figures can offer is present on any network this builds. Consumers read
    these keys with a 0.0 default, which makes an absent attribute score as a
    constant rather than raise, so leaving one out would corrupt a result
    quietly.
    """
    ...
    if G.number_of_nodes() > 0:
        ...
        if True:                        # always; see the note above
            ...
            # Brandes is O(n*m), and at this pipeline's density m grows with
            # n^2, so the cost is roughly cubic in node count. A large network
            # therefore gets sampled sources rather than no attribute at all -
            # consumers read these keys with a 0.0 default, so omitting one
            # would score every node as zero without raising.
