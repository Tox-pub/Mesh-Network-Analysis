...
* **`include_subgraph_pagerank` (Boolean, experimental):** Default `False`. When enabled, PageRank is *additionally* computed on the filtered consensus subgraph (rather than the full co-occurrence graph, which is where `pagerank_centrality` comes from) and scored as a fourth benchmark scorer, `pagerank (subgraph)`. The two measure different things — importance within the curated concept space versus within the whole corpus — so it is reported alongside, never as a replacement. Enabling it requires community detection and relevance scoring to re-run so the extra score column is produced; with the flag off, no column is written and the benchmark simply reports the usual three scorers.
