...
* **`run_ground_truth_analysis` (Boolean):** Master switch for the whole step. **If unset it follows `Use Reference Data`** — on when you are running against the bundled reference corpus (which the bundled ground truth describes), off when you are running your own data (where that ground truth would not apply). Set it explicitly to override. The interactive wizard prompts for it first in this section.
  When enabled, this **also** causes PageRank to be computed on the filtered consensus subgraph (in addition to the whole-corpus `pagerank_centrality`) and scored as a fourth benchmark scorer, `pagerank (subgraph)`. The two measure different things — importance within the curated concept space versus within the whole corpus — so it is reported alongside, never as a replacement. Because that decision changes which centralities are computed, it is read at the **network** step; the wizard therefore offers this flag during `--step network` as well. If the score column is absent the benchmark simply reports the usual three scorers.
